...
window.addEventListener("load",
   function() {
      ...
      for (var i=0; i<formFields.length;i++) {
         ...
         document.getElementById("subButton").oninput =  runSubmit;
      }
...
function validateAddress() {
	...
	if (address.validity.valueMissing) {
		address.setCustomValidity("Enter your name as it appers on the card");
	} else {
...
function validateName() {
   ...
   if (cardName.validity.valueMissing) {
      cardName.setCustomValidity("Enter your name as it appers on the card");
   } else {
...
function sumDigits(numStr) {
   ...
   for (var i=0; i<numStr.length; i++) {
      digitsTotal = digitsTotal + numStr[i]*1;
   }
